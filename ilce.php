...
if($_SERVER['REQUEST_METHOD'] == "GET") {
    ...
    if(isset($_GET["id"])){
		$query = "select * from ilce where id='".$_GET["id"]."'";
	}else if(isset($_GET["ilce"])){
		$query = "select * from ilce where ILCE_ADI_BUYUK='".strtoupper($_GET["ilce"])."'";
	}else if(isset($_GET["il"])){
		$bilgiler = $db->query("select * from il where IL_ADI_BUYUK='".strtoupper($_GET["il"])."'")->fetch_assoc();
		$query = "select * from ilce where IL_ID='".$bilgiler["IL_ID"]."'";
	}else if(isset($_GET["plaka"])) {
		$query = "select * from ilce where IL_ID='".$_GET["plaka"]."'";
	}else{
